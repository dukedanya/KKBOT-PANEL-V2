...
async def buy_plan(callback: CallbackQuery, db):
    ...
    providers = get_enabled_payment_providers()
...
async def _pay_plan_with_balance(
    ...
    *,
    ...
    if existing_payment:
        await callback.answer("⏳ У вас уже есть незавершённый платёж по этому тарифу.", show_alert=True)
        return
...
async def _buy_plan_with_provider_locked(callback: CallbackQuery, db, payment_gateway, panel, bot: Bot):
    ...
    enabled_providers = get_enabled_payment_providers()
    ...
    try:
        ...
        if existing_payment:
            ...
            try:
                ...
                if existing_provider == "telegram_stars":
                ...
                provider_payment_id = get_provider_payment_id(existing_payment)
                pay_url = ""
